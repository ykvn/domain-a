...
spark = SparkSession.builder.appName('merge_revenue_ifrs_dd_overusage').getOrCreate()
# def get_last_partition(hc,table):
#     last_partition = (hc.sql("show partitions "+table)
#                       .orderBy(desc("partition")).select("partition").collect()[0][0])
#     return last_partition.split('=')[1].strip()
...
spark = SparkSession.builder \
    .appName("merge_revenue_ifrs_dd_overusage") \
    .config(conf=conf) \
    .enableHiveSupport() \
    .getOrCreate()
...
QUERY = f"""
    SELECT cast ({event_date} AS DATE) AS trx_date,
...
df.createOrReplaceTempView("temp_merge_revenue_ifrs_dd_overusage")
...
SELECT *
FROM temp_merge_revenue_ifrs_dd_overusage
""")
