...
spark = SparkSession.builder \
    .appName("merge_revenue_ifrs_dd_payu") \
    .config(conf=conf) \
    .enableHiveSupport() \
    .getOrCreate()
...
QUERY = f"""
    SELECT CAST (trx_date as DATE) AS trx_date,
           ...
           content_id AS sigma_business_id,
           '' AS sigma_rules_id,
           '' AS sku,
           '' AS l1_payu,
           '' AS l2_service_type,
    ...
    WHERE event_Date = {event_date}
      ...
      AND SOURCE NOT IN ('LOENA_GOOD_ACT','LOENA_GOOD_PAY','LOENA_REJECT_ACT','LOENA_REJECT_PAY')
    ) a
...
df.createOrReplaceTempView("temp_merge_revenue_ifrs_dd_payu")
...
SELECT *
FROM temp_merge_revenue_ifrs_dd_payu
""")
