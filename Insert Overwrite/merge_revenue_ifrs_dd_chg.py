...
spark = SparkSession.builder \
    .appName("merge_revenue_ifrs_dd_chg") \
    .config(conf=conf) \
    .enableHiveSupport() \
    .getOrCreate()
...
QUERY = f"""
    SELECT cast ( trx_date as DATE) AS trx_date,
           ...
           cast(sum(call_duration) AS int) AS dur,
           cast(sum(event_allowance_consumed) AS int) AS trx,
           cast(sum(total_volume) AS bigint) AS vol,
           cust_id,
           '' AS profile_name,
...
df.createOrReplaceTempView("temp_merge_revenue_ifrs_dd_chg")
...
SELECT *
FROM temp_merge_revenue_ifrs_dd_chg
""")
